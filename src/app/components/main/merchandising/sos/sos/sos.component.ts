...
export class SosComponent implements OnInit {
  ...
  public displayedColumns = ['date', 'merchandiser', 'customerCode', 'customer', 'block_store', 'no_of_Shelves'];
  public apiResponse = {
    pagination: {
      total_records: 0
    }
  ...
  ngOnInit(): void {
    ...
    this.getSosList();
    this.subscriptions.push(this.eventService.on(Events.SOS, ({ request, response }) => {
  ...
  onCloseCriteria() {
    this.advanceSearchRequest = []
    this.eventService.emit(new EmitEvent(Events.CHANGE_CRITERIA, { reset: true, module: Events.SOS, route: '/merchandising/sos' }));
  }
  onChangeCriteria() {
  ...
  getSosList() {
    ...
    this.subscriptions.push(
      this.merService.getSosMainList(this.filterForm.value).subscribe((res) => {
