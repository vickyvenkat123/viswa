...
export class SoaComponent implements OnInit {
  ...
  public displayedColumns = ['date', 'merchandiser', 'customer_code', 'customer', 'no_of_sku'];
  public apiResponse = {
    pagination: {
      total_records: 0
    }
  ...
  ngOnInit(): void {
    ...
    this.getSoaList();
    this.subscriptions.push(this.eventService.on(Events.SOA, ({ request, response }) => {
  ...
  onCloseCriteria() {
    this.advanceSearchRequest = []
    this.eventService.emit(new EmitEvent(Events.CHANGE_CRITERIA, { reset: true, module: Events.SOA, route: '/merchandising/sos' }));
  }
  onChangeCriteria() {
  ...
  getSoaList() {
    this.subscriptions.push(
      this.merService.getSoaMainList(this.filterForm.value).subscribe((res) => {
