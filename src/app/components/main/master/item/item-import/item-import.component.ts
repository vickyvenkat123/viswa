...
export class ItemImportComponent implements OnInit {
  ...
  remove(filesList): void {
    ...
    const file = document.querySelector('#item_file') as HTMLInputElement;
    if (file) {
      file.value = null;
    }
  }
  ...
  mappingfield() {
    this._service.itemMappingfield().subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.isActive = false;
        this.data = res.data;
  ...
  finalImport() {
    this._service
      .submitImportItem({ successfileids: this.successfields, skipduplicate: this.skipduplicate })
      .subscribe((res: any) => {
