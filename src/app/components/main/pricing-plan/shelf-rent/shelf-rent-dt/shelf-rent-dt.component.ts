...
@Component({
  selector: 'app-shelf-rent-dt',
  templateUrl: './shelf-rent-dt.component.html',
  styleUrls: ['./shelf-rent-dt.component.scss']
})
export class ShelfRentDtComponent implements OnInit {

  @Output() public itemClicked: EventEmitter<any> = new EventEmitter<any>();
  @Output() public selectedRows: EventEmitter<any> = new EventEmitter<any>();
  @Input() public isDetailVisible: boolean;
  @Input() public newShelfRentData: any = {};
  public allResData = [];
  public apiResponse = {
    pagination: {
      total_records: 0,
  ...
  public ngOnInit(): void {
    ...
    this.getAllShelfRents();
    // this.dataSource = new MatTableDataSource<ShelfRent>(shelf-rent);
  ...
  getAllShelfRents() {
    this.subscriptions.push(this.apiService.getAllShelfRents(this.filterForm.value).subscribe((shelfRent: any) => {
      this.apiResponse = shelfRent;
      this.allResData = shelfRent.data;
      this.updateDataSource(shelfRent.data);
    }));
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes) {
      if (changes.newShelfRentData && Object.keys(changes.newShelfRentData.currentValue).length > 0) {
        let currentValue = changes.newShelfRentData.currentValue;
        this.newShelfRentData = currentValue;
        this.updateAllData(this.newShelfRentData);
      }
    }
  }
  ...
  public openDetailView(data: any): void {
    ...
    let shelfRentDetails;
    this.apiService.getShelfRent(data.uuid).subscribe(res => {
      shelfRentDetails = res.data;
      this.itemClicked.emit(shelfRentDetails);
