...
export class AddSalesmanLoadFormItemTableComponent implements OnInit, OnChanges {
  ...
  constructor(apiService: ApiService, dataService: DataEditor, formBuilder: FormBuilder, router: Router, route: ActivatedRoute, masterService: MasterService) {
  ...
  onItemSelection(event, i) {
    ...
    this.isStockCheck(this.itemFormControls[i]);
    this.onHandQtyCheck();
...
const ITEM_ADD_FORM_TABLE_HEADS: any[] = [
  ...
  { id: 4, key: 'onhand_qty', label: 'On Hand Qty' },
];
