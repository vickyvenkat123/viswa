...
export class SalesTargetFormComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit(): void {
    this.isEditForm = this.router.url.includes('/target-comission/edit/');
    ...
    this.ownerFormControl = new FormControl('', [Validators.required]);
    this.applyLevelFormControl = new FormControl('item', [Validators.required]);
    this.targetTypeFormControl = new FormControl('quantity', [
      Validators.required,
    ]);
    ...
    this.salesTargetFormGroup = this.formBuilder.group({
      TargetEntity: this.targetOnFormControl,
      ApplyOn: this.applyLevelFormControl,
      TargetType: this.targetTypeFormControl,
      TargetName: this.nameFormControl,
      TargetOwnerId: this.ownerFormControl,
      StartDate: this.startDateFormControl,
      EndDate: this.endDateFormControl,
      TargetVariance: this.rangeTypeFormControl,
      CommissionType: this.commissionTypeFormControl,
      items: this.initItemFormArray(),
      headers: this.initHeaderFormArray(),
    });
    ...
    if (this.isEditForm) {
      ...
      this.pageTitle = 'Edit Target Commission';
      ...
      this.pageTitle = 'Add Target Commission';
  ...
  public goToAllDataList(): void {
    this.router.navigate(['target/target-comission']);
  }
    ...
    if (this.isEditForm) {
      ...
      this.targetService
        ...
        .subscribe((result) => {
          this.toaster.showSuccess(
            'Success',
            'Target Commission has been updated successfuly.'
          );
          this.router.navigate(['/target/target-comission']);
        });
    } else {
      this.targetService
        ...
        .subscribe((result) => {
          this.toaster.showSuccess(
            'Success',
            'Target Commission has been added successfuly.'
          );
          this.router.navigate(['/target/target-comission']);
        });
    }
