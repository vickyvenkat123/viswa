...
@Component({
  selector: 'app-add-pdc',
  templateUrl: './add-pdc.component.html',
  styleUrls: ['./add-pdc.component.scss']
})
export class AddPdcComponent implements OnInit, OnChanges {
  ...
  public ngOnInit(): void {
    this.isAddForm = this.router.url.includes('finance/pdc/add');
  ...
  valuechange(ev) {
    ...
    if (ev.target.value != '') {
      ...
      varienceDiff = value - ev.target.value;
    } else {
      varienceDiff = '';
    }
    ...
    this.collectionData.forEach(element => {
      if (element.collectiondetails.length) {
        element.collectiondetails.forEach(element1 => {
          ...
          element1.cheque_number = element.cheque_number;
          element1.cheque_date = element.cheque_date;
          element1.bank_info = element?.bank_info?.bank_name;
